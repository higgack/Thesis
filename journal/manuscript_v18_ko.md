...
**그림 1.** 하이브리드 본딩의 기반 공정, 구조, 적용 제품(저자 작성; Lau(2021, 2022)의 기술 설명을 바탕으로 재구성).
...
표 6은 출원이 제조공정 기호(H01L21)를 가지는지에 대한 이항 로짓이다. 기술범위가 가장 강한 관련 변수다. 세부 분류가 하나 늘 때마다 제조공정 기호를 가질 승산이 약 27% 높아진다(OR 1.267, p < 0.01). 출원연도의 승산비는 1보다 작아(연간 OR 0.946, p < 0.01), 최근 출원일수록 제조공정 기호를 가질 승산이 낮다. 출원청 가운데는 중국만 미국과 유의하게 달라서, 제조공정 기호를 가질 승산이 65% 높다(OR 1.650, p < 0.05). 모형의 판별력을 나타내는 ROC 곡선 하면적은 0.707로, Hosmer et al.(2013)의 기준에서 받아들일 만한 수준의 하한에 해당한다. 이 모형은 제조공정 기호의 유무만을 묻는다. 경계를 넘는 통합형과 제조공정 기호만 있는 공정 단독형을 가르는 것은 다음 절의 다항 로짓이다.
...
**관측치의 독립성.** 같은 제목의 출원을 한 군집으로 묶은 표준오차에서도 표 9의 네 계수는 모두 1% 수준에서 유지된다. 다만 범위 → 공정 단독형(RRR 0.905)은 p = 0.29로, 일본 → 공정 단독형은 p = 0.06으로 유의성을 잃는다.
...
**표 10.** 가설과 결과 요약(\* p < 0.10, \*\* p < 0.05, \*\*\* p < 0.01).

| 가설 | 예측 | 주요 증거 | 판정 |
|---|---|---|---|
| H1 | 세부 기술이 다양할수록 통합형 | RRR 1.403\*\*\*(표 7); 모든 재추정에서 1.295–1.485\*\*\*(표 9) | 지지(구성적 연관) |
| H2 | 최근 출원일수록 공정 단독형이 드묾 | RRR 0.913\*\*\*(표 7); 기간 더미 0.273\*\*\*/0.101\*\*\*; 재추정에서 0.820–0.935\*\*\*(표 9); 다만 2015년 이전 출원이 30건뿐인 핵심 표본에서는 시간 추세가 식별되지 않음 | 지지 |
| H3a | 중국 출원청 출원은 범위가 좁음 | IRR 0.785\*\*\*(표 5); 모든 재추정에서 0.785–0.808\*\*\*(표 9) | 지지 |
| H3b | 중국 출원청 출원은 공정 단독형이 많음 | RRR 2.687\*\*\*(표 7); 재추정에서 2.721–7.211\*\*\*(표 9); 다만 2023–2024년 제외 시 1.651(비유의) | 제한적 지지(최근 출원 구성에 민감) |
