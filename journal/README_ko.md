...
**v2_ko 2차 수정(유저 지시):** 국문 저널 체재에 맞지 않는 Elsevier 요소 제거 — 하이라이트, 저자 각주 placeholder(^a/*), 원고 메타 노트, 말미 선언문 6종(CRediT·이해상충·연구비·데이터 가용성·AI 사용 선언·감사의 글). 그림 캡션은 이미지 아래 1회만(pandoc 중복 라벨 제거). ⚠ AI 사용 고지는 본문에서 뺐으므로 투고 저널이 요구하면 투고 시스템/표지에서 별도 제출.
