...
반도체 제조는 웨이퍼 위에 소자를 만드는 전공정과, 완성된 웨이퍼를 잘라 조립하는 후공정으로 오랫동안 나뉘어 있었다. 하이브리드 본딩은 이 가운데 후공정에 속하는 접합 기술이다. 그런데 이 기술의 성능은 표면 평탄화나 표면 처리처럼 전공정에서 다루어 온 제조공정 기술에 좌우된다. 그렇다면 두 영역의 기술은 실제로 하나의 특허 안에 함께 담기고 있는가, 그리고 어떤 특허가 그러한가. 이 물음에 답하기 위해 유럽특허청 PATSTAT에서 하이브리드 본딩 관련 특허출원 928건을 추출하였다. 각 출원에 부여된 CPC 기호를 보고, 접합 기술 분류(H01L24)만 가진 출원은 조립 단독형, 제조공정 기술 분류(H01L21)만 가진 출원은 공정 단독형, 두 분류가 함께 부여된 출원은 통합형으로 구분하였다. 분석단위는 개별 특허출원이며, 두 클래스의 공동분류는 제조공정과 접합 영역 사이의 경계 넘기를 포착하는 특허 기반 대리측정치로 쓴다. 통합형이 45.8%로 가장 많았고, 조립 단계 공정을 제외한 좁은 정의를 적용해도 37.2%에 이르렀다. 회귀분석에서는 세 가지가 확인된다. 두 클래스 안에서 더 다양한 세부 기술을 포함하는 출원일수록 통합형일 가능성이 높았는데(조립 단독형 대비 상대위험비 1.40), 두 변수가 같은 CPC 정보에서 만들어지는 만큼 이는 통합형 특허의 구성적 특징으로 해석한다. 또한 최근 출원일수록 공정 단독형으로 분류될 상대적 가능성이 낮았다(2014년 이전 43.6%, 2020–2024년 10.4%). 중국 출원청 출원은 미국 출원청 출원보다 기술범위가 좁았고 이 차이는 표본을 바꾸어도 남았지만, 공정 단독형 편중은 최근 출원에 민감하여 제한적인 결과에 머물렀다. 이러한 결과는 기술융합이 산업과 산업 사이에서만이 아니라 한 산업의 공정 단계 사이에서도 일어난다는 견해와 정합적인 특허 수준의 증거이며, 그러한 융합을 개별 출원 단위에서 측정할 수 있음을 보여준다.

**주제어:** 기술융합; 산업 아키텍처; 하이브리드 본딩; 첨단 패키징; 특허 범위; 다항 로짓

## Abstract

For decades, semiconductor manufacturing has been split between front-end wafer fabrication and back-end assembly. Hybrid bonding sits on the back-end side of that line, yet its performance rests on capabilities that grew up in the front end, above all surface planarisation and surface treatment. Are the two bodies of technology now being combined inside single patents, and if so, in which patents? To answer this we draw 928 hybrid-bonding-related applications from EPO PATSTAT and sort them by the CPC symbols each one carries: assembly-only applications hold H01L24 symbols (bonding technology), process-only applications hold H01L21 symbols (fabrication processes), and integrated applications hold symbols from both classes. The unit of analysis is the individual application, and co-classification in the two classes is used as a patent-based proxy for boundary spanning between the fabrication-process and bonding domains. Integrated applications form the largest group, at 45.8%, and still account for 37.2% when assembly-stage processes are excluded from the definition. Three regression results follow. Applications that cover more diverse technical elements within the two classes are more likely to be integrated (relative-risk ratio 1.40 against assembly-only), a pattern we read as a compositional feature of integrated patents because both variables are built from the same CPC information. More recent filings are less likely to be process-only (from 43.6% before 2015 to 10.4% in 2020–2024). Applications filed at the Chinese office are narrower in scope than those filed at the US office, and this difference survives every change of sample, whereas their tilt towards process-only classification depends on the most recent filings and remains limited evidence. Taken together, the findings offer patent-level evidence consistent with technological convergence across process stages within a single industry, and show that such convergence can be measured application by application.
...
반도체 산업은 반세기 넘게 분업 구조 위에서 움직여 왔다. 트랜지스터는 전공정(front end)에서 웨이퍼 위에 만들어지고, 다 만들어진 웨이퍼는 후공정(back end)으로 넘어가 잘리고 조립되고 포장된다. 전공정이 종합반도체기업(IDM)과 파운드리가 운영하는 대규모 팹에 집중되는 동안(Kapoor, 2013), 패키징과 테스트는 조립·테스트 전문업체(OSAT)로 외주화되었다(Macher and Mowery, 2004; Brown and Linden, 2009). 두 단계 사이의 인터페이스가 비교적 안정적으로 관리되었기에 기업 간 전문화도 가능했다. 그러나 첨단 패키징에 이르러서는 사정이 달라져, 한 단계의 성능이 다른 단계의 공정 역량에 더 직접적으로 의존하기 시작했다.

이 구조가 최근 흔들리고 있는 배경에는 미세화의 한계가 있다. 회로를 더 작게 만드는 일이 갈수록 어려워지자 산업은 다이를 어떻게 나누고, 쌓고, 연결하는가에서 성능 향상의 여지를 찾기 시작했다(Arden et al., 2010; Khan et al., 2018). 그 가운데 가장 까다로운 기술이 하이브리드 본딩이다. 솔더 범프 없이 구리 패드와 주변 산화막을 직접 붙이는 이 기술은 기존 방식으로는 어려운 10 µm 이하의 연결 간격을 가능하게 한다(Lau, 2021, 2022). 문제는 그 성패가 어디에 달려 있는가다. 나노미터 수준의 표면 평탄화, 표면 활성화, 정밀 정렬은 모두 전통적으로 전공정에 속해 온 제조공정 역량이다. 기술적 상호의존이 이렇게 커지자 선도 파운드리가 첨단 패키징 역량을 내부에 갖추는 움직임도 나타났다(Lau, 2022; KnowMade, 2024).

본 연구가 묻는 것은 이러한 경계의 흐려짐을 개별 특허 수준에서 관찰할 수 있는가이다. 출발점은 서로 구분되던 기술 영역이 겹치고 결합하는 과정을 다룬 기술융합 문헌이다(Rosenberg, 1963; Kodama, 1992; Curran and Leker, 2011; Sick and Bröring, 2022). 다만 이 문헌의 실증 연구가 주로 다루어 온 것은 정보통신과 가전(Hacklin et al., 2009), 식품과 제약(Bröring et al., 2006), 나노기술과 바이오기술(No and Park, 2010)처럼 서로 다른 산업이나 기술 분야가 겹치는 경우였다. 한 산업 안에서 가치사슬의 단계를 가로지르는 융합은 드물게만 다루어졌다. 가장 가까운 선례는 가치사슬 상·하류 기업 간의 특허 인용을 분석한 Karvonen과 Kässi(2012)이지만, 이 연구도 하나의 기술이 공정 단계의 경계를 넘는지를 특허 단위에서 검정하지는 않았다. 그런데 누가 무엇을 만들고 누가 가치를 가져가는가를 정하는 산업 아키텍처(Jacobides et al., 2006)에 압력을 가하는 융합은 바로 이런 종류의 융합이다.

분석에는 유럽특허청(EPO)의 PATSTAT에서 추출한 하이브리드 본딩 관련 특허출원 928건과, 각 출원에 부여된 협력적 특허분류(CPC) 기호를 쓴다. 접합·인터커넥트 기술(H01L24)과 반도체 제조공정 기술(H01L21)의 기호가 한 출원에 함께 부여되어 있으면 그 출원을 통합형, 곧 경계를 넘는 특허로 부르고, 접합 기호만 있는 출원은 조립 단독형, 제조공정 기호만 있는 출원은 공정 단독형으로 구분한다. 공동분류 자체는 융합을 포착하는 표준적인 특허 지표다(Curran and Leker, 2011; Kwon et al., 2020). 본 연구의 새로움은 이를 두 산업 사이가 아니라 한 산업을 가로지르는 경계에 적용한다는 점, 그리고 분야 수준의 지수로 집계하는 대신 개별 특허의 유형으로 재구성한다는 점에 있다. 연구 질문은 셋이다. 하이브리드 본딩 특허에서 제조공정과 접합 기술의 공동분류는 얼마나 일반적이며 시간에 따라 어떻게 변했는가. 어떤 특허 특성이 공동분류 유형과 관련되는가. 그리고 이러한 유형과 기술범위는 출원청별로 어떻게 다른가.

미리 말하면, 통합형은 출원의 45.8%로 가장 많고 좁은 정의에서도 37.2%에 이른다. 더 다양한 세부 기술을 포함하는 출원일수록 통합형이었고, 최근 출원일수록 공정 단독형이 드물었으며, 중국 출원청 출원은 기술범위가 좁았다. 이 결과로 본 연구는 세 방향에서 기여한다. 개념적으로는 융합 이론을 산업 간 사례에서 산업 내 공정 단계 간 사례로 넓히고, 이를 반도체 산업 아키텍처 문헌(Langlois and Steinmueller, 1999; Kapoor and Adner, 2012; Kapoor, 2013)과 잇는다. 방법론적으로는 새로운 융합지수를 하나 더 보태는 대신 공동분류를 개별 특허의 유형으로 재구성하여, 분야 수준 지수가 평균 속에 묻어 버리는 특허 간 이질성과 그 관련 요인을 추정할 수 있게 한다. 마지막으로 이 설계는 두 개의 CPC 클래스와 하나의 분류 규칙, 표준 추정량만으로 이루어져 있어, 하류 단계가 상류 역량에 의존하기 시작한 다른 공정기술에도 그대로 옮겨 쓸 수 있다.
...
여기서 말하는 융합은 수직통합과 같은 것이 아니다. 수직통합 문헌(Macher and Mowery, 2004; Kapoor, 2013)이 묻는 것은 어느 기업이 어느 단계를 직접 수행하는가, 곧 조직의 경계다. 반면 융합 문헌이 묻는 것은 서로 다른 영역의 기술이 하나의 발명 안에서 결합되는가, 곧 지식의 경계다(Kodama, 1992; Curran and Leker, 2011). Hacklin et al.(2010)의 단계 모형을 빌리면 기술의 융합은 산업의 융합에 앞선다. 본 연구가 특허에서 관찰하려는 것도 이 앞선 단계, 다시 말해 기업의 조직적 결정 이전에 또는 그 이면에서 진행되는 기술의 결합이다. 그러므로 산업 아키텍처는 여기서 직접 검정하는 결과변수가 아니라 결과를 해석하는 상위의 이론적 맥락이고, 수직통합은 직접 측정하지 않는 후속 현상으로 남겨 둔다.
...
**출원청.** 어디에 출원하는가에는 누가 무엇을 위해 발명하는가가 드러난다. 단계별 추격의 논리(Lee and Lim, 2001), 중국 특허에 관한 증거(Hu and Jefferson, 2009; Dang and Motohashi, 2015), 반도체 가치사슬에서 중국의 위치(Grimes and Du, 2022)를 함께 놓고 보면, 중국 출원청 출원은 특정 제조 단계의 확보에 집중되어 범위가 좁고 공정 단독형이 많으리라는 예상이 가능하다. 물론 이 예상에는 중간 논리가 있어야 한다. 중국 특허의 증가, 보조금, 추격이라는 사실이 그 자체로 좁은 CPC 범위와 공정 단독형을 예측해 주지는 않기 때문이다. 후발국의 특허 활동이 생산 역량의 국산화, 공정별 병목의 해소, 특정 제조 단계의 권리 확보에 집중된다면, 하나의 출원이 포괄하는 기술 요소는 시스템 전체의 통합보다 개별 공정 문제의 해결에 가까워진다. 그렇다면 두 클래스 안의 CPC 범위는 좁고 공정 단독형의 비중은 높을 것이다. 이는 중국 특허 선행연구가 직접 내린 결론이 아니라, 단계별 추격의 논리를 본 연구의 특허 구성에 적용해 끌어낸 예상이다. 범위와 유형은 서로 다른 모형으로 검정되므로 가설도 나누어 세운다.
...
본 연구의 이론적 개념은 공정 단계 사이의 기술융합이다. 관찰하는 현상은 제조공정 기술과 접합·인터커넥트 기술이 한 출원에 함께 담기는 것이고, 조작적 지표는 H01L21과 H01L24의 공동분류다(표 2). 여기서 분명히 해 둘 점이 있다. 본 연구는 H01L21을 전공정 그 자체로, H01L24를 후공정 그 자체로 보지 않는다. H01L21에는 웨이퍼 제조공정만이 아니라 조립 단계의 제조공정(패키지 부품 제조, 실장·봉지, 리드 부착, 다이싱)과 공정 장치도 들어 있고, H01L24가 조직적 의미의 후공정 산업 경계를 그대로 나타내는 것도 아니다. 그럼에도 두 클래스의 공동분류를 대리측정치로 쓰는 근거는, 하이브리드 본딩의 성능을 좌우하는 웨이퍼 준비, 표면 처리, 평탄화 및 관련 제조공정이 H01L21에 다수 포함되고 접합·인터커넥트 기술은 H01L24에 모여 있다는 데 있다. 표본의 H01L21 계열 기호 1,636건을 뜯어 보면 웨이퍼 제조공정이 905건(55.3%), 공정 장치가 328건(20.0%), 조립 단계 공정이 403건(24.6%)이다. 조립 단계 공정을 제조공정으로 세지 않는 좁은 정의로 다시 추정한 결과는 제5.4절에 보고하며, 회귀분석의 결론은 달라지지 않는다.
...
**범위의 기계적 부분.** 통합형은 정의상 두 클래스의 기호를 하나씩은 가지므로 범위가 2 이상일 수밖에 없다. 이 최소 기호 수의 차이가 결과를 만드는지 보기 위해 각 보유 클래스의 첫 기호를 뺀 조정 지표로 다시 추정해 보면, 통합형의 RRR은 1.403에서 1.262로 줄지만 정적 연관 자체는 남는다(부록 B). 그렇다고 이 지표가 기계적 연관을 완전히 걷어낸 독립적 검정인 것은 아니다. 조정 지표 역시 유형 정보를 이용해 만들어지기 때문이다. 그래서 이 결과를 H1의 결정적 증거로 쓰지 않으며, H1은 인과효과가 아니라 통합형 특허의 구성적 특성으로 해석한다.
...
**관할권이 분류에 남기는 흔적.** 출원청 간 차이가 나타난 곳은 통합형이 아니라 공정 단독형과 기술범위였다. 중국 출원청에 진입한 발명 포트폴리오는 범위가 좁고, 특정 제조공정에 집중된 출원이 상대적으로 많다. 적층 전체를 포괄하기 전에 특정 제조 단계부터 확보하는 추격 단계의 출원 양상(Lee and Lim, 2001)과 정합적인 패턴이고, 중국 특허의 양적 팽창과 보조금 정책에 관한 증거(Hu and Jefferson, 2009; Dang and Motohashi, 2015)와도 어긋나지 않는다. 그렇다고 이를 추격 전략의 직접적인 증거로 읽을 수는 없다. 출원청 계수는 관할권의 인과효과가 아니라 각 출원청에 제출된 발명 포트폴리오의 조건부 구성 차이이며, 출원인과 패밀리 정보가 통제되지 않았기 때문이다. 출원청 효과에는 출원인의 국적과 유형, 같은 발명의 해외 출원 여부, 심사 기준과 분류 관행이 뒤섞여 있고, 공정 단독형 편중은 최근 출원 구성에 민감한 제한적 결과이기도 하다. 그보다 확실하게 말할 수 있는 것은, 범위와 시점을 통제하면 통합형의 비중 자체는 출원청 간에 다르지 않다는 점이다. 본 표본에서 출원청별 차이는 공동분류형보다 한 영역에만 분류된 출원에서 더 두드러지게 관찰되었다. 한국과 대만은 범위와 통합형에서 미국과 비슷한데, 미국 출원인과 정면으로 경쟁하는 메모리 제조사와 파운드리의 위치를 생각하면 자연스러운 결과다.

### 6.2 실무적·정책적 함의

자료가 출원인의 유형을 구분하지 않으므로 실무적 함의는 가능성의 수준에서만 제시한다. 조립·테스트 전문업체의 성과나 특허 경쟁력을 직접 비교한 연구는 아니다. 그러나 상당수 출원이 제조공정과 접합 기술을 함께 포함한다는 결과는, 하이브리드 본딩에서는 접합 장비를 잘 다루는 것만으로는 충분하지 않고 표면 평탄화, 오염 제어, 표면 화학, 정렬까지 아우르는 인접 공정 지식이 중요해질 수 있음을 시사한다. 독립적 산업 분석이 파운드리, 종합반도체기업, 라이선싱 기업을 특허 활동을 주도하는 출원인으로 꼽는 것(KnowMade, 2024)도 같은 방향을 가리킨다. 파운드리와 종합반도체기업의 패키징 내재화를 이 결과가 직접 입증하는 것은 아니다. 다만 제조공정과 접합 기술의 공동분류가 빈번하다는 사실은 두 공정 영역을 잇는 지식 범위가 하이브리드 본딩 관련 특허 활동에서 중요하다는 점을 보여주며, 이는 Kapoor와 Adner(2012)가 말한 지식 경계의 논리와 정합적이다.

정책 면에서는 관할권 비교가 시사하는 바가 있다. 중국 출원청 표본의 좁은 클래스 내 CPC 범위는 견고하게 관찰되었다. 그 원인이 보조금 정책(Dang and Motohashi, 2015)인지, 출원인 구성인지, 기술 단계나 출원 선택인지는 본 자료로는 가릴 수 없다. 한국 출원청의 계수는 기준 모형에서 유의하지 않으므로 한국에 대한 함의는 표본 전체의 패턴에서 끌어낸 것임을 밝혀 둔다. 차세대 HBM을 하이브리드 본딩에 기대고 있는 한국의 메모리 산업에 이 결과가 시사하는 바는, 패키징 경쟁력이 패키징 노하우만이 아니라 평탄화, 표면 화학, 정렬 계측 같은 제조공정 역량, 그리고 이를 하나의 특허출원에서 통합적으로 보호하는 능력에 달려 있을 수 있다는 점이다.
...
결과는 견고성에 따라 세 층으로 나누어 읽을 수 있다. 가장 견고한 것은 세 가지다. 본 연구의 검색식으로 식별된 하이브리드 본딩 관련 출원에서 제조공정과 접합 기술의 공동분류는 45.8%에 이르렀고, 보수적 정의로도 37.2%여서 예외적인 현상이라 보기 어렵다. 공정 단독형의 상대위험은 표본과 정의를 어떻게 바꾸어도 최근 출원일수록 낮았다. 중국 출원청 출원은 미국 출원청 출원보다 두 클래스 안의 CPC 범위가 좁았고, 이 차이 역시 모든 표본에서 유지되었다.
