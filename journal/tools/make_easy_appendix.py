...
import os
r16 = json.load(open('robustness/results_v16.json')) if int(VER) >= 16 and os.path.exists('robustness/results_v16.json') else None
if r16:
    r['mnl_core'] = r16['core']['mnl']; r['nb_core'] = r16['core']['nb']
...
specs = [('mnl_base','기준'),('mnl_adj','조정 범위'),('mnl_dedup','동일 제목 중복 제거'),('mnl_strict','좁은 전공정 정의'),
         ('mnl_clean','무관 용법 제외'),('mnl_le2022','2023–2024년 제외'),('mnl_2010','2010년 이후')]
if r16: specs.insert(5, ('mnl_core','핵심 표본'))
out = []
out.append('## 부록\n')
out.append('### 부록 A. 기술범위 모형의 비교\n')
out.append('표 A1은 본문의 음이항 모형을 선형 회귀(OLS) 및 Poisson 모형과 나란히 보인 것이다. 부호와 유의성은 세 모형에서 같다. 다만 기술범위는 분산이 평균보다 훨씬 큰 카운트 변수여서(음이항의 과산포 모수 α = 0.268, 표준오차 0.020; Poisson 대비 우도비 검정 χ² = 596.3, p < 0.001), Poisson의 표준오차는 실제보다 작게 추정된다. 예컨대 일본 계수의 표준오차는 Poisson에서 0.080, 음이항에서 0.133이다. 이 때문에 본문에서는 음이항을 쓴다.\n')
out.append('**표 A1.** 기술범위의 관련 요인: 모형 비교(N = 928; 출원청 기준 = 미국; 괄호 안은 표준오차; Poisson·음이항 계수의 지수가 IRR; \\* p < 0.10, \\*\\* p < 0.05, \\*\\*\\* p < 0.01).\n')
...
def mnl_table(eq, label, tno):
    ...
    NUMS.extend(f"{abs(r[key]['llf']):,.1f}" for key,_ in specs)
...
if r16:
    ...
    for v in c.values(): NUMS.extend(f"{x:.2f}" for x in v)
