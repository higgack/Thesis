...
def draw(lang):
    ...
    tiers = [
        (T('기반 공정', 'Enabling processes'), T('(CPC H01L21 제조공정 클래스)', '(CPC H01L21, fabrication processes)'),
         [T('CMP 평탄화', 'CMP planarisation'), T('세정·파티클 제어', 'Cleaning / particle control'), T('표면 활성화', 'Surface activation'), T('정렬', 'Alignment'), T('저온 어닐링', 'Low-temperature anneal')]),
        ...
        (T('적용 제품', 'Applications'), '',
         [T('CMOS 이미지센서', 'CMOS image sensor'), '3D NAND', 'HBM', T('로직 적층', 'Logic stacking'), 'MicroLED']),
    ]
    fig, ax = plt.subplots(figsize=(9.2, 6.4)); ax.set_xlim(0, 100); ax.set_ylim(0, 100); ax.axis('off')
    n = len(tiers); h = 17.5; gap = (100 - n * h) / (n + 1)
    for i, (title, sub, items) in enumerate(tiers):
        y0 = 100 - gap * (i + 1) - h * (i + 1)
        ax.add_patch(FancyBboxPatch((4, y0), 92, h, boxstyle='round,pad=0.4,rounding_size=1.5', fc=LIGHT, ec=EDGE, lw=1.2))
        ax.text(6, y0 + h - 3.2, title, fontsize=12, fontweight='bold', color=NAVY, va='center')
        if sub: ax.text(6 + len(title) * 2.4 + 2, y0 + h - 3.2, sub, fontsize=8.5, color='#555', va='center')
        k = len(items); w = (88 - (k - 1) * 1.6) / k
        for j, it in enumerate(items):
            x = 6 + j * (w + 1.6)
            ax.add_patch(FancyBboxPatch((x, y0 + 2.2), w, 8.2, boxstyle='round,pad=0.3,rounding_size=1.2', fc='white', ec='#333', lw=1.0))
            ax.text(x + w / 2, y0 + 6.3, it, ha='center', va='center', fontsize=9.2 if k < 5 else 8.4)
        if i < n - 1:
            ax.add_patch(FancyArrowPatch((50, y0 - 0.4), (50, y0 - gap + 0.4), arrowstyle='-|>', mutation_scale=16, color=NAVY, lw=1.6))
    lbl = [T('가능하게 함', 'enables'), T('구현', 'realised as'), T('적용', 'used in')]
    for i in range(n - 1):
        y0 = 100 - gap * (i + 1) - h * (i + 1)
        ax.text(52.5, y0 - gap / 2, lbl[i], fontsize=8.5, color=NAVY, va='center')
    fig.tight_layout(); fig.savefig(f"figures/fig_framework_psp{'' if lang == 'ko' else '_en'}.png", dpi=200); plt.close(fig)
