...
LAB = {'US': '미국', 'CN': '중국', 'KR': '한국', 'TW': '대만', 'JP': '일본', 'EP': 'EPO', 'WO': 'PCT', 'Other': '기타'}
NAVY = '#1f3b5c'
...
ax.set_xlabel('출원연도'); ax.set_ylabel('출원 건수'); ax.set_xlim(1966, 2025)
ax.axvspan(2022.5, 2024.5, color='#bbbbbb', alpha=0.35, lw=0); ax.text(2023.5, max(vals) * 0.93, '공개 시차', ha='center', fontsize=8, color='#444')
fig.tight_layout(); fig.savefig('figures/fig2_applications_by_year_ko.png'); plt.close(fig)

# Fig 3: OLS residuals vs fitted
fig, ax = plt.subplots(figsize=(6.5, 4.2))
ax.scatter(r['ols_resid']['fitted'], r['ols_resid']['resid'], s=8, color=NAVY, alpha=0.7)
ax.axhline(0, color='#b22222', lw=1); ax.set_xlabel('선형 예측값'); ax.set_ylabel('잔차')
fig.tight_layout(); fig.savefig('figures/fig3_ols_residuals_ko.png'); plt.close(fig)
...
ax.set_xticks(x); ax.set_xticklabels([LAB[o] for o in OFF]); ax.set_xlabel('출원청'); ax.set_ylabel('예측 기술범위(서브그룹 수)')
fig.tight_layout(); fig.savefig('figures/fig4_predicted_breadth_ko.png'); plt.close(fig)

# Fig 5: ROC
fig, ax = plt.subplots(figsize=(4.8, 4.5))
ax.plot(r['roc']['fpr'], r['roc']['tpr'], color=NAVY, lw=1.5); ax.plot([0, 1], [0, 1], '--', color='#888', lw=1)
ax.set_xlabel('1 − 특이도'); ax.set_ylabel('민감도'); ax.set_xlim(0, 1); ax.set_ylim(0, 1)
fig.tight_layout(); fig.savefig('figures/fig5_roc_ko.png'); plt.close(fig)
...
ax.set_xticks(x); ax.set_xticklabels([LAB[o] for o in OFF]); ax.set_xlabel('출원청'); ax.set_ylabel('공정 단독형 평균 예측확률'); ax.set_ylim(0, None)
fig.tight_layout(); fig.savefig('figures/fig6_predicted_process_only_ko.png'); plt.close(fig)
...
bottom = np.zeros(3)
for t, lab, col in [('Process', '공정 단독형', '#8c8c8c'), ('Integrated', '통합형(경계 넘기)', NAVY), ('Assembly', '조립 단독형', '#9ec1e6')]:
    ...
    bottom += sh
ax.set_xticks(range(3)); ax.set_xticklabels([f'{q}\n(N = {N[q]})' for q in periods]); ax.set_ylabel('비중(%)'); ax.set_ylim(0, 100)
ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.18), ncol=3, frameon=False, fontsize=8)
fig.tight_layout(); fig.savefig('figures/fig7_period_shares_ko.png'); plt.close(fig)
