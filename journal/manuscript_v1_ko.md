...
^a [대학명] 기술경영학과, 서울, 대한민국

\* 교신저자. E-mail: [기관 이메일]

*저널 투고용 원고(영문판 `manuscript_v1.md`의 국문 번역). 대상 저널: Technovation / Technological Forecasting and Social Change (Elsevier) 양식. 원고 유형: 연구논문. 분량: 본문 약 7,500단어 상당(참고문헌·표·부록 제외).*

---

## 하이라이트 (Highlights)

- 하이브리드 본딩 특허로 가치사슬 *내부*의 기술융합을 검정한다.
- PATSTAT 928건 중 63.6%가 접합(H01L24)과 함께 전공정(H01L21)을 청구한다.
- CPC 서브클래스가 하나 늘 때마다 전공정 청구 승산이 약 27% 높아진다.
- 순수 전공정 청구는 시간이 갈수록 줄지만, 경계를 넘는 청구는 줄지 않는다.
- 중국 출원청 출원은 공정 지향적이고, 일본 출원청 출원은 거의 그렇지 않다.
...
논문의 구성은 다음과 같다. 제2절은 이론적 배경을 검토하고 가설을 도출한다. 제3절은 실증 맥락인 하이브리드 본딩 생태계를 기술한다. 제4절은 데이터와 방법을, 제5절은 결과를 제시한다. 제6절은 이론적·실무적 함의와 한계를 논의하고, 제7절은 결론을 맺는다.
...
## 3. 연구 맥락: 하이브리드 본딩 생태계

### 3.1 기술

하이브리드 본딩은 유전체 안에 구리 패드가 패터닝된 두 평탄화 표면을 직접 접촉시켜, 상온에서 유전체가 유전체에 접합되고 저온 어닐링 후 구리가 구리에 접합되게 함으로써 영구적 인터커넥트를 만든다. 솔더 범프가 없기 때문에 인터커넥트 피치는 마이크로범프 플립칩보다 한 자릿수 작은 1µm 이하로 내려갈 수 있고, 접합 계면은 후공정 배선(BEOL)의 연장에 가까울 만큼 얇다(Lau, 2021, 2022). 공정은 두 형태로 존재한다. 웨이퍼-투-웨이퍼(W2W) 접합은 웨이퍼 전체를 접합하며 CMOS 이미지센서와 3D NAND에서 먼저 상용화되었다. 다이-투-웨이퍼(D2W) 접합은 개별 다이를 웨이퍼에 부착하며, 칩렛의 이종 집적, 고대역폭 메모리 적층, 3D 로직에 요구되는 형태다. 두 형태 모두에서 수율을 결정하는 단계—서브나노미터 거칠기로의 화학적 기계연마, 플라즈마 또는 화학적 표면 활성화, 파티클 제어, 서브미크론 정렬—는 CPC의 의미에서 전공정 역량이다. 이들은 접합·인터커넥트 클래스인 H01L24가 아니라 반도체 장치 제조를 위한 공정·장치 클래스인 H01L21에 속한다(부록 A).

### 3.2 출원 동학과 생태계 구조

동일한 PATSTAT 추출에 대한 두 가지 기술적 분석이 배경을 제공한다(전체 내용은 보충자료 S1). 그림 1은 *공정* 기술인 하이브리드 본딩과 이에 가장 뚜렷하게 의존하는 *제품* 인 고대역폭 메모리(HBM)의 연간 출원을 비교한다. 두 시계열은 2010년대 후반까지 함께 움직였다. 2019년에는 거의 같았고(56건 대 41건), 2020년에 갈라지기 시작하였다(71건 대 45건). 2021년 하이브리드 본딩 출원은 118건으로 급증하여 전년 대비 약 66% 증가한 반면, HBM 출원은 30건 안팎에서 정체하였다. 기술의 개발이 제품보다 앞서 달린 것이며, 이는 가능화(enabling) 공정에서 기대되는 바다.

출원인 구조는 집중되어 있다. 출원인 수준 분석에서 상위 10개 출원인이 순위 특허의 58.5%를 차지하며(그림 2), Intel과 Adeia—직접접합 인터커넥트의 원천인 Ziptronix와 Invensas를 흡수한 라이선싱 기업—가 선두이고, TSMC와 프랑스 연구기관 CEA가 뒤를 잇는다. 더 큰 하이브리드 본딩 특허 집합에 대한 독립적 산업 분석은 TSMC, Adeia, YMTC, Intel, Samsung을 선도 출원인으로 지목하며(KnowMade/Yole Group, 2024), 이는 여기서 관찰된 순서와 부합한다. 소규모 출원인 중 전력반도체·방산 기업의 존재는 이 기술의 응용이 로직과 메모리를 넘어 확장됨을 보여준다.

Narin et al.(1987, 1997)과 H. Ernst(2003)를 따라 산출한 인용 기반 지표는 세 전략 유형을 분리한다(표 1). *원천기업*—Ziptronix와 그 핵심 발명가—은 평균 270회 이상 인용되고 무인용이 없는 특허를 보유하며, 표본에서 가장 높은 과학연계 점수와 가장 넓은 시장 커버리지를 갖는다. 이들은 생태계의 나머지가 라이선스하거나 우회 설계해야 하는 진입장벽을 구축하였다. *선도 제조사* TSMC는 높은 인용 영향력과 매우 낮은 과학연계 점수를 결합하는데, 이는 엔지니어링 주도 공정개발의 프로필이다. 그 현시 기술우위(Soete, 1987)는 인터커넥트, 비아 충전, 다마신, 평탄화 코드—즉 패키징에 투입된 H01L21 전공정 기술—에 집중되어 있으며, 이는 SoIC 플랫폼이 체현하는 패턴이다. *추격 진입자* YMTC는 Adeia의 원천기술을 라이선스한 뒤 자체 W2W "X-Stacking" NAND 공정을 개발하였다. 26건의 특허가 모두 인용되고, 인용은 출원 2년 후에 정점에 이르며, 과학연계 비율은 표본에서 가장 높은 편이다. 반면 Intel은 가장 많은 특허(144건)를 보유하지만 주요 출원인 중 평균 인용 영향력이 가장 낮고, Samsung의 무인용 비율은 3분의 1을 넘는다. 그림 3은 생태계를 기술 네트워크로 요약하여 이들 행위자, 하이브리드 본딩이 요구하는 동행 공정(실리콘 관통 전극, 평탄화, 다이싱, 표면 준비), 그리고 그것이 가능케 하는 구조와 제품을 매핑한다.

**표 1.** 주요 출원인의 생태계 지표(출처: 동일 PATSTAT 추출에 대한 출원인 수준 분석; 보충자료 S1).
...
| YMTC(메모리, 추격) | 49.2 | 0% (26건) | 논문인용특허비율 48.5% | 11.69 / 99.7% | 라이선스 후 자체 W2W NAND 개발; 인용 정점 2년차 |
...
*주:* 과학연계는 특허당 비특허문헌 인용의 평균 수이며, YMTC는 논문을 인용한 특허의 비율만 가용하다. "—"는 원 분석에서 해당 출원인에 대해 보고되지 않은 통계량이다.

이러한 생태계 수준의 그림이 특허 수준 분석의 동기가 된다. 패키징 특허가 전공정 코드에 기반한 선도 파운드리, 모두가 우회 설계해야 하는 과학 기반 특허를 가진 원천기업, 라이선싱에서 독자 공정개발로 빠르게 이동한 추격 진입자가 보인다. 이 전략들이 *어떤 특허* 가 전/후공정 경계를 넘는지에 체계적 흔적을 남기는가가 논문의 나머지가 다루는 질문이다.

## 4. 데이터와 방법

### 4.1 데이터

데이터는 유럽특허청의 PATSTAT Global 2025년판에서 가져왔다(European Patent Office, 2025). 후보 레코드는 CPC 분류—H01L21(반도체 장치 제조를 위한 공정·장치)과 H01L24(반도체 바디의 연결·분리 배치) 및 그 인덱싱 코드—로 검색한 뒤, 위양성을 제거하기 위해 공정 키워드(*hybrid bonding*, *direct bonding*, *Cu–Cu*, *copper to copper*, *metal-oxide bond*)의 AND 결합으로 필터링하였다(부록 A, 그림 A1). 추출 결과는 출원–CPC 레코드 5,277건이며 각 레코드는 하나의 출원과 하나의 CPC 기호를 짝짓는다. 출원 수준으로 응집하면 1968–2024년에 출원된 고유 출원 928건이 된다. 모든 기호는 H01L(반도체 장치) 아래에 있으며, 그 안에서 레코드는 본 연구에서 전/후공정 경계를 정의하는 두 클래스 H01L21과 H01L24로 나뉜다.
...
기술범위는 평균 5.69개 서브클래스(표준편차 4.07, 범위 1–21)다. 미국 출원청이 382건으로 가장 많고, 중국(175), PCT 경로(113), EPO(83), 대만(75), 한국(58), 일본(26), 기타(16)가 뒤를 잇는다. 그림 4는 출원연도 프로필을 보여준다. 2010년 이전에는 활동이 미미하다가 2010년대 후반에 가파르게 상승하여 2022년경 정점에 이르고, 2023–2024년의 하락은 실제 감소가 아니라 최근 출원이 공개되기까지의 시차를 반영한다. 전략 유형별로는 425건(45.8%)이 통합형, 338건(36.4%)이 조립 단독형, 165건(17.8%)이 공정 단독형이며, 합계 590건, 즉 63.6%가 적어도 하나의 제조공정 기호를 보유한다.
...
표 3은 기술범위 모형을 요약하며, OLS·로버스트 OLS·Poisson·음이항 추정치의 전체 비교는 부록 B(표 B1)에 제시한다. 선형 벤치마크에서 연도 추세는 양이고 유의하지만(0.070, p < 0.01) 모형의 설명력은 낮다(R² = 0.033). 잔차는 등분산을 기각하고(Breusch–Pagan p < 0.001) 카운트 결과변수 특유의 띠 무늬와 깔때기 형태를 보인다. 음이항의 과산포 모수는 크고 정밀하게 추정되며(ln α = −1.316, α ≈ 0.27, p < 0.001), Poisson 대비 우도비 검정은 결정적이고, AIC는 Poisson의 5,477에서 음이항의 4,883으로 낮아진다. 따라서 음이항을 작업 모형으로 삼는다.

**표 3.** 기술범위의 결정요인: 음이항 추정치(N = 928; 미국 출원청이 기준; 괄호 안은 표준오차; \* p < 0.10, \*\* p < 0.05, \*\*\* p < 0.01).
...
몇 가지 점검이 결과의 강건성에 관련된다. 첫째, 카운트 모형의 선택이 범위 결과를 좌우하지 않는다. 연도와 중국 출원청 효과의 부호와 유의성은 OLS, 로버스트 OLS, Poisson, 음이항 명세에서 동일하며(표 B1), 표준오차가 보정되면서 정밀도를 잃는 것은 일본 출원청 계수뿐이고, 이 때문에 이를 10% 수준에서 보고한다. 둘째, 2023–2024년의 출원 감소는 공개 시차의 인공물이다. 연도 추세가 1968–2024년 전 기간에 걸쳐 추정되고 모형이 연도를 선형으로 통제하므로, 절단은 추정 추세의 방향보다 최근 카운트의 수준에 영향을 미친다. 다만 공정 단독형의 감소를 절단된 연도로 외삽하는 것은 경계한다. 셋째, 다항 결과는 무관 대안의 독립성 가정에 따라 조립 단독형 기준 대비 상대적 경향으로 읽으며, 그 가정을 요구하지 않는 이항 로짓이 범위·시간·중국 출원청에 대해 같은 질적 결론을 준다.
...
*산업 아키텍처와 추격이 청구에 남기는 흔적.* 네 번째 함의는 특허 수준 결과를 전략 문헌과 연결한다. 관할권 대비는 공정 단독형 갈래에 집중되어 있으며 제2.4절에서 제시한 해석과 부합한다. 중국 출원청 출원은 좁고 불균형하게 공정 단독형이다. 이는 통합된 적층 전체를 청구하기 전에 특정 제조 단계를 내재화하는 추격 자세의 프로필이며(Lee and Lim, 2001), YMTC에 관한 생태계 증거—원천기업의 기술을 라이선스한 뒤 유난히 빠른 인용 정점을 보이는 독자 W2W 공정을 개발—는 같은 자세를 기업 수준에서 예시한다. 일본 출원청 출원은 넓고 사실상 공정 단독형이 없다. 이는 단일 단계 주변이 아니라 전체 공정 흐름에 걸쳐 특허를 내는 장비·소재 공급사와 부합한다. 한국과 대만은 범위와 통합형 갈래에서 미국 기준 근처에 있으며, 이는 미국 출원인과 정면 경쟁하는 메모리 제조사와 파운드리에 대해 개연적이다. 이들의 높은 공정 단독형 경향(대만은 10% 수준에서 유의)은 TSMC의 다마신·평탄화 코드에서의 현시 기술우위가 예시하는 파운드리 주도의 전공정 패키징 역량 내재화와 부합한다. 종합하면 이 패턴들은 산업의 *아키텍처*—누가 어느 단계를 어디에서 점유하는가—가 *어떤* 발명이 경계를 넘는지를 형성함을 보이며, 이는 기업의 통합 선택이 산업 아키텍처를 형성한다는 Kapoor(2013)의 주장을 기업이 작성하는 청구항 수준으로 확장한 것이다.
...
**이형규:** 개념화, 방법론, 데이터 큐레이션, 정식 분석, 원고 초안 작성, 원고 검토·편집, 시각화.

## 이해상충 선언

저자는 본 논문에 보고된 연구에 영향을 미쳤을 것으로 보일 수 있는 알려진 경쟁적 재정 이해관계나 개인적 관계가 없음을 선언한다.

## 연구비

본 연구는 공공, 상업 또는 비영리 부문의 연구비 지원기관으로부터 특정 지원금을 받지 않았다.

## 데이터 가용성

특허 데이터는 EPO PATSTAT Global(2025년판)에서 라이선스 하에 추출되었으며 저자가 재배포할 수 없다. 추출 질의(CPC 코드와 키워드 필터, 부록 A)와 추정 스크립트는 요청 시 저자로부터 제공받을 수 있다. 보충자료 S1은 표 1과 그림 1–2에 요약된 출원인 수준 지표를 보고한다.

## 작성 과정에서의 생성형 AI 및 AI 보조 기술 사용 선언

본 연구의 준비 과정에서 저자는 문헌 정리, 언어 교정, 원고 구조화를 보조하기 위해 AI 언어모델을 사용하였다. 모든 실증 분석, 해석, 결론은 저자 자신의 것이다. 이 도구를 사용한 후 저자는 필요에 따라 내용을 검토·편집하였으며 출판물의 내용에 대해 전적으로 책임진다.
...
하이브리드 본딩 모집단은 두 단계로 검색하였다. (1) H01L21과 H01L24 및 그 인덱싱 코드에 대한 CPC 1차 필터; (2) *hybrid bonding*, *direct bonding*, *Cu–Cu*, *copper to copper*, *metal-oxide bond* 를 결합한 키워드 필터(그림 A1). HBM 비교 모집단(약 2,330 레코드)은 H01L21, H01L24, H01L2224, H01L2924와 키워드 *high bandwidth memory*, *HBM*, *wide I/O memory*, *stacked memory die*, *memory stack* 으로 같은 절차에 따라 구축하였으며, 그림 1의 기술적 비교에만 사용한다.

**그림 A1.** 하이브리드 본딩 모집단의 검색 전략(CPC 1차 필터와 키워드 2차 필터).

![그림 A1. 검색 전략](figures/figA1_search_strategy.png)

## 부록 B. 기술범위 모형의 전체 비교

**표 B1.** 명세별 기술범위 결정요인(N = 928; 미국 출원청이 기준; 괄호 안은 표준오차; \* p < 0.10, \*\* p < 0.05, \*\*\* p < 0.01).
...
**그림 1.** 하이브리드 본딩(공정)과 고대역폭 메모리(제품)의 연간 특허출원, 2010–2024. 출처: 동일 PATSTAT 추출에 대한 출원인 수준 분석(보충자료 S1). 2023–2024년의 감소는 공개 시차를 반영한다.

![그림 1](figures/fig1_filing_trends_hb_vs_hbm.png)

**그림 2.** 하이브리드 본딩 특허 수 기준 상위 10개 출원인(출원인 수준 분석; 자회사는 현재 모회사로 통합, 예: Ziptronix와 Invensas는 Adeia). 출처: 보충자료 S1.

![그림 2](figures/fig2_top10_applicants.png)

**그림 3.** 하이브리드 본딩 기술 생태계: 핵심 공정, 동행 공정, CPC 클래스, 가능화된 구조와 제품, 주요 출원인(인용 영향력·과학연계·시장 커버리지 지표 병기), 출원청 경향. 표 1의 지표와 표 3–5의 추정치로부터 결정론적으로 구성.

![그림 3](figures/fig3_ecosystem_network.png)

**그림 4.** 출원연도별 하이브리드 본딩 출원, 1968–2024(N = 928). 2023–2024년의 하락은 공개 시차 인공물이다.

![그림 4](figures/fig4_applications_by_year.png)

**그림 5.** 출원청별 예측 기술범위(음이항, 다른 공변량은 평균; 95% 신뢰구간).
...
**그림 7.** 출원청별 공정 단독형 전략의 예측 확률(다항 로짓, 다른 공변량은 평균; 95% 신뢰구간).
