...
```
┌─────────────────────────────────────────────────────────────────┐
│                     사용자 (Telegram / Obsidian)                │
└──────────────┬──────────────────────────────────┬───────────────┘
               │ 질문 (/ask, /deep, …)            │ 열람 (/wiki, vault)
               ▼                                  ▼
┌──────────────────────────────────────┐ ┌────────────────────────┐
│  Layer 3: 검색·답변 (agent/tools.py) │ │  P5: Telegram 커맨드   │
│                                      │ │  /wiki /wiki_today     │
│  search_my_brain(query)              │ │  /wiki_status /wiki_run│
│    ├─ wiki_context(query)            │ │  /wiki_off /wiki_on    │
│    │  └─ 매칭? → 합성 페이지 prepend │ │  /wiki_backfill        │
│    └─ hybrid() RAG → top-K 청크     │ └────────────────────────┘
│                                      │
│  ★ wiki miss/error → RAG 그대로     │
│  ★ WIKI_QUERY_FIRST=0 → RAG only    │
└──────────────────────────────────────┘
               │ 답변 조합
               ▼
┌──────────────────────────────────────────────────────────────────┐
│                                                                  │
│  Layer 2: LLM Wiki 합성 (store/wiki.py)                         │
│  ┌──────────────────────────────────────────────────────────┐    │
│  │  SecondBrain/Wiki/                                       │    │
│  │    삼성전자.md  ←  flash 머지 (purpose="wiki")           │    │
│  │    HBM.md       ←  출처 표기 + ⚠️ 모순 추적              │    │
│  │    TSMC.md      ←  git 버전 관리 (diff·revert 가능)      │    │
│  │    …                                                     │    │
│  └──────────────────────────────────────────────────────────┘    │
│  ★ Chroma/meta.db 절대 불변 — vault .md + wiki_*.json 만 씀    │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
               │ wiki_context(): 토픽명 매칭 (₩0)
               │ read_page(): 합성 페이지 반환 (₩0)
               ▼
┌──────────────────────────────────────────────────────────────────┐
│                                                                  │
│  Layer 1: 원본 코퍼스 (기존 RAG — 완전 독립)                    │
│  ┌─────────────────┐  ┌──────────────────────────────┐          │
│  │  Chroma (벡터)  │  │  Obsidian per-doc 노트        │          │
│  │  임베딩 청크    │  │  요약 · 메타 · 원문 마크다운  │          │
│  │  hybrid() 검색  │  │  data/meta.db (SQLite)        │          │
│  └─────────────────┘  └──────────────────────────────┘          │
│  ★ Wiki ON/OFF 무관하게 byte-for-byte 동일                      │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
```
               ...
               ▼                  ▼                   │
    ┌──────────────────┐  ┌────────────────┐          │
    │ meta.upsert_doc  │  │ wiki.enqueue() │          │
    │ vector.upsert    │  │  ₩0, LLM 0    │          │
    │ obsidian.write   │  │  중요도 게이트 │          │
    └──────────────────┘  │  (≥600자 요약) │          │
    │ Chroma+meta.db      └───────┬────────┘          │
    │ (임베딩 비용 발생)          │                    │
    │                              ▼                   │
    ...
    │                    ▼                   ▼
    │          ┌──────────────┐    ┌──────────────────┐
    │          │ Wiki/<토픽>.md│    │  wiki_index.json  │
    │          │ vault에 저장 │    │  토픽→doc_ids 매핑│
    │          └──────┬───────┘    └──────────────────┘
    │                 │
    │                 ▼
    │          ┌──────────────────┐
...
```
src/
├── config.py               WIKI_* 플래그 12개
├── store/
│   ├── wiki.py             핵심 엔진 (enqueue, run_batch, wiki_context, …)
│   ├── cost.py             비용 추적 (purpose="wiki" 분리 집계)
│   ├── obsidian.py         commit_subtree() — vault git 관리
│   └── meta.py             docs_since() — 백필용 코퍼스 조회
├── ingest/
│   └── pipeline.py         wiki.enqueue() hook (L664, try/except 래핑)
├── agent/
│   └── tools.py            wiki_context() → prepend (L170, try/except 래핑)
└── bot.py
    ├── _wiki_batch_job()   야간 배치 + 다이제스트 + 알람 (L11285)
    ├── cmd_wiki*            /wiki 커맨드 7개
    └── run_repeating()      24h 스케줄 (KST 03:00 앵커)

data/                        (Docker bind-mount, 컨테이너 내 /app/data)
├── wiki_queue.json          대기열 (atomic write, .bak 폴백)
├── wiki_index.json          토픽→doc_ids 매핑
├── wiki_last_run.json       마지막 배치 결과 (/wiki_today)
└── wiki_disabled            킬스위치 파일 (/wiki_off → touch, /wiki_on → rm)

<vault>/SecondBrain/Wiki/    Obsidian vault 내 위키 페이지
├── 삼성전자.md
├── HBM.md
├── TSMC.md
└── …
