...
- **Phase 1 (파싱 고도화 + 비용 게이트)**:
  - **페이지 triage (ePapyrus 방식)** — 합성 전에 PyMuPDF 싸구려 신호
    (글자수·텍스트/이미지 커버리지·표·폼·블록밀도)로 페이지를
    SKIP / TEXT_ONLY / OCR_NEEDED / LLM_NEEDED 4버킷 분류 → TEXT는 무료
    `get_text`, OCR/LLM은 필요한 페이지만. 노트 인입 **비용 게이트**.
    거의 드롭인(PyMuPDF 이미 의존, `import pymupdf`≡`import fitz`). cost
    민감 노트엔 1순위. (봇엔 미적용 — OCR가 큰 비용 아니라 ROI 낮음.)
  - 디지털 PDF 구조가 더 필요하면 `opendataloader-pdf` A/B 투입
    (회사리포트 A/B와 동일 방식).
...
- **오디오 복습 (Gemini TTS)**: NotebookLM의 "오디오 개요"에서 착안 —
  노트를 음성으로 들으며 복습해 체화를 강화. NotebookLM-mcp(브라우저
  자동화)는 8GB VM 부적합·비공식·ToS 회색이라 **도구는 미채택**; 대신
  **Gemini TTS로 직접 생성**(노트당 짧은 요약 음성). 저빈도라 비용 작음.
  Phase 0 검증 후 검토.
- **노트→슬라이드 export (선택)**: ppt-master류로 노트를 발표/교습용
  .pptx로. "가르치며 배우기(teach-to-learn)"는 체화에 유효하나 본 축
  (인입·되새김질)과는 별개라 후순위. 필요해질 때만.
