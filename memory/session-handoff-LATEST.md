...
**유저 선택: v1_ko 계열.** → **`journal/manuscript_v24_ko.md`(쉬운 판, +docx, 부록 A–C 포함 단일 파일)가 현재 최신 국문 핵심본**; v24 = 4.4 분석방법 상세화(표 1–11, 그림 1–6), v23 = breadth_adj 정의 명확화, v22 = 선별 검토 반영(초록·주제어·IIA·CI) + 그림 전부 재출력, v21 = AI 검사 잔여 14블록 2차 문체 재작성(AI 13%, 표절 1%), v20 = 1차 문체 재작성(26%→19%), v19 = 최종 평가 반영(그림 1 재작성 포함; README v19 참조; 외부 검토 5종 모두 반영 완료). 남은 일: 저널용 영문 축약본은 v9 기반 TASM판이 구판이므로 v19 기준으로 재작성 필요(유저 요청 시), v17 = 계량분석 검토 반영, v16 = 외부 구성·논리 검토 반영(README v16 항목 참조; 표 1–10, 그림 1–6; 신규 추정 `tools/robustness_v16.py`), v15 = PATSTAT 자리표시자 제거, v14 = 외부 인용 검증 반영(README 참조). v13 = 저자 줄 축약, v12 = 저자 줄 축약 전, v11 = 출원인 문구 수정 전, v10 = 같은 본문 + 별도 보충자료 S1·S2. **규칙(유저): 본문을 고칠 때마다 새 버전 번호**(v13…).; v9는 심사 방어용 상세판(가정·강건성 전부 수록). 빌드·검증: `python3 tools/make_easy_appendix.py N && python3 tools/make_easy_ko.py N`(원본 `sources/ko_vN/`; N≥16은 results_v16.json도 사용, 참고문헌 덮어쓰기 `refs_extra.md`, 추가 숫자 풀 `numbers_extra.txt`). 유저 요구: 쉽게, 가정 적게, 원자료를 못 보는 심사관·교수가 이해하도록. v9 = 7관점 심사(151건) 반영(`review_v8_ko.md`); 모든 수치는 `tools/robustness_v9.py` → `robustness/results_v9.json`에서 생성. 저널 타깃: TASM(영문)·기술혁신연구(국문), `target_journals_ko.md`. **양식본 완료(09-24):** `manuscript_v9_ko_JTI.md/.docx`(기술혁신연구 임시 양식, `tools/make_kr_journal_v9.py`) · `manuscript_v9_en_TASM.md/.docx` + `supplement_v9_en.md/.docx`(TASM, 영문 번역은 `scratchpad/en_v9/*.md`에서 `tools/make_en_journal_v9.py`로 조립·감사; 번역 절 원본은 `journal/sources/en_v9/`). 모든 v9 DOCX 재생성: `tools/build_v9_docx.sh`. 남은 일: (a) 투고규정 원문 확인(TASM 분량·초록 제한, 기술혁신연구 규정·HWP 여부), (b) 영문 원어민 교정, (c) TASM 분량 초과 시 본문 축약(현 ~11,750단어), (d) 저자 확인 항목(PATSTAT 판본·추출일, ITRS URL, 이메일). v6=HB.csv 재추정, v7=정합성·가독성, v8=학기 과제(KR/KR_Final) 수치 대조(전부 일치) + 그림 1(개념도, D2W 오기 수정)·그림 3(OLS 잔차)·표 3(출원청별 유형 분포) 추가 + AI 문체 제거('명세'→'모형 설정', 기울임·줄표 제거). 출원인 정보는 구할 수 없음(유저 확인) → 관할권 해석은 '경쟁·보호의 장'. Rev1_ko는 보류. 원자료 `journal/data/`(gitignore)는 컨테이너 재생성 시 유저에게 다시 받아야 함.
