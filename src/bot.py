...
def main():
    ...
    if app.job_queue:
        ...
        # v8: 대학원 복원(실수로 삭제) — 종목/산업/전략/스터디/반도체/AI/
        #     공부/대학원/부동산/그외 10종, 대학원은 공부 뒤 배치 (2026-07-24)
        # 2026-08-26: 투자론 추가 — 새 노트는 synth가 바로 분류하지만
        #     _NOTES_CAT_VERSION은 8에 그대로 둔다. 사용자 요청이 "백필하지
        #     말라"였고, 9로 올리면 기존 노트 1,200여 개가 전부 재분류돼
        #     (Flash-Lite 호출 1,200회 + 경계가 미묘한 기존 분류가 흔들림)
        #     원하지 않는 이동이 생긴다. 기존 노트는 대시보드 배지 클릭으로
        #     직접 옮기면 되고, 그 수동 선택은 category_locked=1이라 나중에
        #     누가 버전을 올려도 보존된다. 분류기 프롬프트가 바뀌었다고
        #     반사적으로 여기를 올리지 말 것.
        _NOTES_CAT_VERSION = 8
