...
async def _check_yt_dlp_health(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg = (
            ...
            f"YouTube가 추출기를 또 바꾼 듯. 이미지는 매 배포마다 yt-dlp "
            f"<b>nightly</b>를 자동으로 끌어오고(EJS+Deno로 JS 챌린지 처리), "
            f"nightly가 보통 stable보다 먼저 fix를 담음.\n\n"
            f"해결:\n"
            f"• 아무 커밋이나 푸시 → auto_pull 재빌드 시 최신 nightly가 "
            f"자동 반영됨 (별도 수동 명령 불필요).\n"
            f"• 그래도 계속 실패면 nightly가 아직 fix를 못 담은 것 → "
            f"하루이틀 뒤 재배포. 또는 Deno/EJS 런타임 이슈일 수 있으니 "
            f"<code>docker logs --tail 30 thesis-bot-1</code> 확인.\n\n"
            f"그 사이 YouTube 학습은 transcript_api 가 한 번씩 성공할 때만 "
            f"들어가고 나머지는 stub 으로 떨어져."
        )
