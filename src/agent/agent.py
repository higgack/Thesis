...
질문이 특정 회사 분석을 요구할 때 본문 분석에 더해 다음 섹션을 반드시 추가 (자료에 근거 있을 때만; 없으면 해당 섹션 생략):
  ...
  📊 가이던스 분산 (자료에 같은 회사·같은 연도 가이던스가 2건 이상일 때만)
    표 형식 예시:
      2023A  매출 258.9조 (실적) | OP 6.6조 (실적)
      2024E  매출 280~300조 (중앙값 290, YoY +12.0%) | OP 25~35조 (중앙값 30, YoY +354%)
      2025E  매출 310~330조 (중앙값 320, YoY +10.3%) | OP 40~50조 (중앙값 45, YoY +50%)

    [참여 분석가 N건]
      • 브로커리지명 / 애널리스트명 / YYYY.MM.DD 발행 [출처N]
      • NH투자증권 / 김XX / 2026.04.20 [1]
      • 신한투자증권 / 박XX / 2026.04.25 [3]
      • (애널리스트명 또는 발행일 미상이면 "—" 로 표기. 학습일자 절대 사용 금지 —
         반드시 자료 메타데이터/본문에서 보이는 원본 리포트 발행일 (report_date /
         자료 본문 1페이지 상단 표기) 만 사용)
      • 브로커리지명은 자료의 title/source/내용에서 추출 (예: "NH투자증권 삼성전기"
         파일명, "Source: SK증권" 본문 헤더). 추출 불가 시 [출처N] 만 표기.

    - YoY% = (당해 - 전년) / 전년 * 100, 소수 첫째자리.
    - 1건뿐인 연도도 분산 없이 표 한 줄로 포함 (range 대신 단일값).

  📈 매출 차트 (mermaid xychart-beta 블록 1개)
  → 본문 흐름상 매출/실적을 논하는 섹션 직후에 inline 배치할 것. 답변 맨 끝 X.

  📈 영업이익 차트 (mermaid xychart-beta 블록 1개)
  → 매출 차트 직후 또는 OP 논의 섹션 직후 inline 배치.

  📈 분기 추이 차트 (분기 데이터 4개 분기 이상 있을 때만, 매출/OP 각 1개 추가)
  → 본문 분기 실적 섹션 직후 inline 배치.

  xychart-beta 엄격 문법 (어기면 mermaid.ink/kroki 400 — 차트 깨짐):
    - 차트는 매출용 / 영업이익용 별도 블록으로 분리 (단일 y축이라 매출과 OP를 한
      차트에 섞으면 OP가 0 근처에 깔려 안 보임 — 가독성 망함).
    - 차트당 데이터 시리즈는 최대 3개 (bar 1, line 2). 다중 분석가 케이스에서
      bar=중앙값, line2개=max/min. 단일 자료 케이스는 bar 1개만.
    ...
    - 데이터 없는 시점이 있으면 두 가지 중 하나:
        ...
        (b) 그 시리즈 자체를 차트에서 제외 (예: 2023 max 없으면 max line 빼고
             min line + bar만)
    - 다중 자료 분산 표시: line은 최대 2개 (max, min). 더 그리면 가독성↓
    ...
    - 분기 차트 x-axis 형식: `["1Q24", "2Q24", "3Q24", "4Q24", "1Q25"]` (큰따옴표 필수)

  예시 — 다중 자료 매출 차트:
    ```mermaid
    xychart-beta
      title "매출 추이 (조원, 분석가 3건)"
      x-axis ["2023A", "2024E", "2025E"]
      y-axis "조원"
      bar [258.9, 290, 320]
      line [258.9, 300, 330]
      line [258.9, 280, 310]
  ...
  예시 — 분기 매출 차트:
    ...
    xychart-beta
      title "분기 매출 (조원)"
      x-axis ["1Q24", "2Q24", "3Q24", "4Q24", "1Q25"]
      y-axis "조원"
      bar [71.9, 75.0, 80.0, 82.0, 79.0]
    ```
