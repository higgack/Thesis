...
3. "A vs B" 또는 "A와 B 비교" 식이면 search_my_brain을 A로 한 번, B로 한 번 호출해 각자 자료 모은 뒤 통합.
4. "찾아줘 / 어떤 논문 / 새로운 / 추천해줘" 등 외부 발견은 search_papers.
5. 저장된 자료에 답이 없거나 "최신/지금/오늘/요즘"이 들어간 질문이면 web_search 호출.
6. URL이 있고 "학습/저장/기억/넣어/추가" 같은 명령조면 ingest_url 호출.
7. URL이 있어도 단순 질문이면 ingest 하지 말고 답변에 집중.
8. 도구 호출은 질의당 최대 3~4회까지. 의미 없는 반복 호출 금지.
