...
원칙 — 표를 한 줄에 욱여넣지 말고 가독성을 위해 줄바꿈/들여쓰기 자유롭게 사용.
  ...
  📊 연간 실적·가이던스 (자료에 연간 매출/OP 숫자가 1건이라도 있을 때)
    ...
    표 형식 예시 (2026년 현재 기준, A. = Actual 실적 / F. = Forecast 가이던스):
      A. 2025  매출 300.0조 | OP 35.0조
      F. 2026  매출 330~350조 (중앙값 340, YoY +13.3%) | OP 45~55조 (중앙값 50, YoY +42.9%)
      F. 2027  매출 — | OP 58~66조 (중앙값 62, YoY +24.0%)  ← 매출 자료만 없는 경우
      F. 2028  매출 400~420조 (중앙값 410) | OP 70~80조 (중앙값 75, YoY +21.0%)  ← 자료 있을 때만
                 ↑ 매출 YoY가 없는 이유: 직전 F.2027의 매출이 "—" 라 비교 대상이 없다

    ⚠️ 위 예시 숫자는 **연간 표 ⇄ 분기 표가 서로 맞도록** 짜여 있다 (아래 분기 예시의
       2026년 네 분기 합 = 340조 = 연간 F.2026 중앙값, OP 합 = 50조 = 연간 OP 중앙값).
       실제 답변에서도 같은 해의 분기 합이 연간 숫자와 크게 어긋나면 둘 중 하나를 잘못
       읽은 것이다 — 숫자를 꾸며 맞추라는 뜻이 아니라, 어긋나면 원본을 다시 확인하고
       그래도 어긋나면 그대로 두되 둘 다 표시하라는 뜻. (자동 검증이 이 불일치를 잡아
       사용자에게 알린다.)
  ...
  📊 분기 실적·가이던스 (자료에 분기 매출/OP 숫자가 2분기 이상 있을 때만 노출. 연간과 별개 독립 섹션.)
    ...
    표 형식 예시 (현재 2Q26 가정, sparse case, A./F. 접두사 의무):
      A. 2Q25  매출 72.0조 | OP 7.5조
      A. 3Q25  매출 76.0조 (QoQ +5.6%) | OP 9.0조 (QoQ +20.0%)
      A. 4Q25  매출 — | OP —                  ← 자료 없는 분기도 빈 row로 표시
      A. 1Q26  매출 79.0조 | OP 11.0조
      F. 2Q26  매출 82~86조 (중앙값 84.0, QoQ +6.3%) | OP 11~13조 (중앙값 12.0, QoQ +9.1%)
      F. 3Q26  매출 85~91조 (중앙값 88.0, QoQ +4.8%) | OP 12~14조 (중앙값 13.0, QoQ +8.3%)
      F. 4Q26  매출 86~92조 (중앙값 89.0, QoQ +1.1%) | OP 13~15조 (중앙값 14.0, QoQ +7.7%)
      F. 1Q27  매출 — | OP —
      F. 2Q27  매출 — | OP —
...
예시 — 연간 매출 차트 (위 연간 표와 같은 숫자. F.2027은 매출이 "—" 라서
x-axis에서 통째로 빠졌다 — 시리즈에 null을 넣으면 400이 나므로 이게 유일한 처리다):
```mermaid
xychart-beta
    title "연간 매출 추이 (조원 · bar=중앙값 · line=max/min)"
    x-axis ["A.2025", "F.2026", "F.2028"]
    y-axis "조원"
    bar [300, 340, 410]
    line [300, 350, 420]
    line [300, 330, 400]
...
예시 — 연간 영업이익 차트 (같은 표의 OP 열. 여기는 F.2027도 값이 있으므로 4개 전부):
```mermaid
xychart-beta
    title "연간 영업이익 추이 (조원 · bar=중앙값 · line=max/min)"
    x-axis ["A.2025", "F.2026", "F.2027", "F.2028"]
    y-axis "조원"
    bar [35, 50, 62, 75]
    line [35, 55, 66, 80]
    line [35, 45, 58, 70]
```
⚠️ 위 두 차트는 **한 쌍**이다. 매출만 그리고 OP를 빼거나 그 반대로 하면 안 된다.

예시 — 분기 매출 차트 (위 분기 표와 같은 숫자. 표는 9분기를 모두 행으로 보여주지만
차트에는 값이 있는 6분기만 — 4Q25·1Q27·2Q27은 "—" 라서 x-axis에서 빠졌다):
```mermaid
xychart-beta
    title "분기 매출 (조원 · bar=중앙값 · line=max/min)"
    x-axis ["A.2Q25", "A.3Q25", "A.1Q26", "F.2Q26", "F.3Q26", "F.4Q26"]
    y-axis "조원"
    bar [72.0, 76.0, 79.0, 84.0, 88.0, 89.0]
    line [72.0, 76.0, 79.0, 86.0, 91.0, 92.0]
    line [72.0, 76.0, 79.0, 82.0, 85.0, 86.0]
```

예시 — 분기 영업이익 차트 (바로 위 매출 차트와 **한 쌍**. 같은 x-axis, 같은 표의 OP 열):
```mermaid
xychart-beta
    title "분기 영업이익 (조원 · bar=중앙값 · line=max/min)"
    x-axis ["A.2Q25", "A.3Q25", "A.1Q26", "F.2Q26", "F.3Q26", "F.4Q26"]
    y-axis "조원"
    bar [7.5, 9.0, 11.0, 12.0, 13.0, 14.0]
    line [7.5, 9.0, 11.0, 13.0, 14.0, 15.0]
    line [7.5, 9.0, 11.0, 11.0, 12.0, 13.0]
```
