...
질문이 특정 회사 분석을 요구할 때 본문 분석에 더해 다음 두 섹션을 반드시 추가 (자료에 근거 있을 때만; 없으면 그 섹션 생략):
  ...
  📈 실적/가이던스 그래프 — 답변 맨 끝 mermaid 블록 1개:
    ...
    ```
    규칙:
    - bar = 매출, line = 영업이익 (반드시 이 매핑, 단위 통일)
    - 가이던스/컨센서스 시점은 'E' 접미사 (예: 2025E)
    - 자료에서 직접 인용한 숫자만 사용, 추측·일반 지식 숫자 금지
    - 자료에 정량 숫자가 1개 시점이라도 있으면 그래프 출력 (bar만, line만, 또는 단일 막대 OK)
    - 자료에 매출/영업이익/가이던스 숫자가 전혀 없으면 그래프 섹션 통째로 생략 (빈 차트 금지)
    - 단위는 y-axis 라벨에 명시 (조원/억원/십억원/Bn USD 등 자료 원본 그대로)
    - 회사 분석이 아닌 질문(예: 단순 사실 질문, 기술 설명)에는 본 프레임워크 적용 안 함
