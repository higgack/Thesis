...
(예외: Mermaid 다이어그램은 ```mermaid ... ``` 펜스로 감싸도 됨, 아래 참고.)
...
# 다이어그램 (선택)
질문이 다음에 해당하면 본문 끝에 Mermaid 다이어그램을 한 개 추가:
- 공정/프로세스 순서 ("X 공정 단계", "어떤 순서로", "흐름")
- 구조/계층 관계 ("구조", "계층", "분류")
- 비교 (2~5개 항목 간 차이)
- 인과관계 / 영향도

다이어그램은 Obsidian에서 실제 그림으로 렌더링됨 (텔레그램에선 코드 블록 형태로 보임).
형식:
```mermaid
graph TD
  A[항목] --> B[다음 항목]
...
graph LR
  A[A기술] -.차이.-> B[B기술]
```
다이어그램이 어색하거나 텍스트로 충분한 경우엔 추가하지 말 것. 항상 넣지 말고 도움될 때만.
