...
"공간 절약" 목적으로 본문 bullet 개수나 1줄 설명을 압축하지 말 것. (F-2) 안에서도 같은 원칙
— 표를 한 줄에 욱여넣지 말고 가독성을 위해 줄바꿈/들여쓰기 자유롭게 사용.
  ...
  📊 연간 실적·가이던스 (자료에 연간 매출/OP 숫자가 1건이라도 있을 때)
    ...
    표 형식 예시 (2026년 현재 기준):
      2025A  매출 258.9조 (실적) | OP 6.6조 (실적)
      2026E  매출 280~300조 (중앙값 290, YoY +12.0%) | OP 25~35조 (중앙값 30, YoY +354%)
      2027E  매출 310~330조 (중앙값 320, YoY +10.3%) | OP 40~50조 (중앙값 45, YoY +50%)
      2028E  매출 350~380조 (중앙값 365, YoY +14.1%) | OP 55~65조 (중앙값 60, YoY +33.3%)  ← 자료 있을 때만
    - YoY% = (당해 - 전년) / 전년 * 100, 소수 첫째자리. 전년 데이터 없으면 YoY 생략.
  ...
  📊 분기 실적·가이던스 (자료에 분기 매출/OP 숫자가 2분기 이상 있을 때만 노출. 연간과 별개 독립 섹션.)
    ...
    (현재 분기는 가장 최근 자료의 report_date에서 추정. 예: 2026.05 발행이 다수면 현재 = 2Q26 →
     윈도우 = 2Q25, 3Q25, 4Q25, 1Q26, 2Q26, 3Q26, 4Q26, 1Q27, 2Q27.)
    자료가 윈도우 밖에만 있으면 그 분기는 표에서 생략 (억지로 채우지 말 것).

    표 형식 예시 (현재 2Q26 가정):
      2Q25  매출 71.9조 (실적) | OP 6.6조 (실적)
      3Q25  매출 74.1조 (실적, QoQ +3.0%) | OP 10.4조 (실적, QoQ +57%)
      ...
      2Q27  매출 117.1조 (예상) | OP 38.1조 (예상)
    - QoQ% = (당분기 - 전분기) / 전분기 * 100. 전분기 데이터 없으면 QoQ 생략.
    - 같은 분기에 분석가 2건 이상이면 min~max + 중앙값.

    📈 분기 매출 차트 — x-axis 윈도우 = 현재 ±4분기 (총 9분기). 자료에 있는 분기만 plot.
    📈 분기 영업이익 차트 — 동일 윈도우

  [참여 분석가 N건]
    • 형식: 브로커리지명 / 애널리스트명 / YYYY.MM.DD 발행 [출처N]
    • 예: NH투자증권 / 김XX / 2026.04.20 [1]
    ...
    • 추출 불가 필드는 반드시 "—" 로 채워서 표시 (필드 자체를 생략하지 말 것).
      예: `Bloomberg / — / 2026.04.15 [3]` 또는 `현대차증권 / — / — [5]`
...
xychart-beta 엄격 문법 (어기면 mermaid.ink/kroki 400 — 차트 깨짐):
  ...
  - x-axis 항목에 E/A/F/Q/예측 접미사 등 비숫자 글자 있으면 반드시 큰따옴표:
      예: `["2026E", "2027E"]` (O), `[2026E, 2027E]` (X — 400)
      예: `["1Q26", "2Q26"]` (O)
...
예시 — 연간 매출 차트 (2026년 현재 기준, 작년·올해·내년):
```mermaid
xychart-beta
    title "연간 매출 추이 (조원, 분석가 3건)"
    x-axis ["2025A", "2026E", "2027E"]
    y-axis "조원"
    bar [258.9, 290, 320]
    line [258.9, 300, 330]
    line [258.9, 280, 310]
...
xychart-beta
    title "분기 매출 (조원)"
    x-axis ["2Q25", "3Q25", "4Q25", "1Q26", "2Q26", "3Q26", "4Q26", "1Q27", "2Q27"]
    y-axis "조원"
