...
필드를 적극 활용해서 풍부하게 작성. 상위 N개는 깊게, 나머지는 1-2줄로.

  📌 주요 논문 (상위 5-7개)
  각 논문 형식:
    📄 <번호>. [제목 한국어 번역] (YYYY) [출처N]
        저자: First et al. · 학회/저널: <venue> · (가능 시) 인용 N회
        <abstract 기반 3-5줄 핵심 요지>:
          • 방법론·접근법 (어떻게)
          • 주요 결과·수치 (무엇)
          • 기여·의의 (왜 중요)
        → DOI: 10.x/y · PDF: <url>  (있는 경우만)

  📌 추가 관련 논문 (나머지 7-8개)
  각 논문 1-2줄:
    • [제목] (YYYY) — 한 줄 핵심 설명 [출처N]

  - abstract 필드가 빈 논문은 제목 + venue + year 만 표시 (짧게).
  - 인용수 (citations) 는 S2 결과에만 있음. 0 또는 없으면 표기 생략. 100+ 이면 강조 가치 있음.
  - 한국어 번역 제목 + 원어 제목 (출처 legend) 둘 다 노출 (legend는 자동 처리).
  - 추측 추가 정보 (모델 사전지식) 절대 금지 — abstract 에서 안 보이는 내용은 쓰지 말 것.
