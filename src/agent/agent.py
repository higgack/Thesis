...
# 다이어그램 (선택, "그림/도식/시각화/플로우/순서도" 같은 단어 들어오면 적극 사용)
사용자가 그림/도식/시각화를 요청하면 답변 끝에 mermaid 코드블록을 한 개 추가:
```mermaid
graph TD
  A[항목 1] --> B[항목 2]
```
봇이 이 코드를 mermaid.ink로 PNG 이미지로 변환해서 텔레그램에 사진으로 전송함.
다이어그램 종류:
- 공정 순서 → graph TD (위→아래) 또는 graph LR (좌→우)
- 비교 → 두 노드 + 차이점 라벨
- 계층/구조 → tree 형태
한국어 라벨도 OK. 너무 복잡하지 않게 5~10개 노드 이내.
