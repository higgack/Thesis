...
연간/분기 데이터를 별도 하위 블록으로 모두 노출 — 둘 다 있으면 둘 다 보여줌.
  ...
  📊 연간 실적·가이던스 (자료에 연간 매출/OP 숫자가 1건이라도 있을 때)
    ...
    표 형식 예시:
      ...
      2024A  매출 300.9조 (실적, YoY +16.2%) | OP 32.7조 (실적, YoY +395%)
      2025E  매출 310~330조 (중앙값 320, YoY +6.4%) | OP 40~50조 (중앙값 45, YoY +37.6%)
      2026E  매출 350~380조 (중앙값 365, YoY +14.1%) | OP 55~65조 (중앙값 60, YoY +33.3%)
    - YoY% = (당해 - 전년) / 전년 * 100, 소수 첫째자리. 전년 데이터 없으면 YoY 생략.
    - 1건뿐인 연도는 단일값 (range 대신).
    - 2건 이상이면 min~max + 중앙값 표시.

    📈 연간 매출 차트 (mermaid xychart-beta 블록 1개)
    📈 연간 영업이익 차트 (mermaid xychart-beta 블록 1개)

  📊 분기 실적·가이던스 (자료에 분기 매출/OP 숫자가 2분기 이상 있을 때만 노출. 연간 데이터와 별개로 독립 섹션.)

    표 형식 예시:
      1Q24  매출 71.9조 (실적) | OP 6.6조 (실적)
      2Q24  매출 74.1조 (실적, QoQ +3.0%) | OP 10.4조 (실적, QoQ +57%)
      ...
      1Q26  매출 117.1조 (예상) | OP 38.1조 (예상)
    - QoQ% = (당분기 - 전분기) / 전분기 * 100. 전분기 데이터 없으면 QoQ 생략.
    - 같은 분기에 분석가 2건 이상이면 min~max + 중앙값.

    📈 분기 매출 차트 (mermaid xychart-beta 블록 1개)
    📈 분기 영업이익 차트 (mermaid xychart-beta 블록 1개)
...
xychart-beta 엄격 문법 (어기면 mermaid.ink/kroki 400 — 차트 깨짐):
  ...
  - 데이터 없는 시점이 있으면 두 가지 중 하나:
      ...
      (b) 그 시리즈 자체를 차트에서 제외 (예: 2023 max 없으면 max line 빼고 min line + bar만)
...
예시 — 연간 매출 차트:
  ...
  xychart-beta
    ...
    x-axis ["2023A", "2024A", "2025E", "2026E"]
    y-axis "조원"
    bar [258.9, 300.9, 320, 365]
    line [258.9, 300.9, 330, 380]
    line [258.9, 300.9, 310, 350]
  ```
...
예시 — 분기 매출 차트:
  ...
  xychart-beta
    title "분기 매출 (조원)"
    x-axis ["1Q24", "2Q24", "3Q24", "4Q24", "1Q25", "2Q25"]
    y-axis "조원"
    bar [71.9, 74.1, 79.1, 75.8, 79.0, 82.4]
  ```
