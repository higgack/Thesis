...
CATEGORY는 노트 내용의 종류를 '종목'/'산업'/'전략'/'반도체'/'AI'/'부동산'/
'공부'/'그외' 중 하나로만 적는다. 우선순위(위에서부터 먼저 적용):
...
- 공부 = 위 어디에도 안 속하는, 특정 기업·투자와 무관한 순수 지식·기술·
  과학·학문 학습.
- 그외 = 일반 거시경제·통화·정책·통계 등 위 일곱 어디에도 안 맞는 것.
단어 하나만, 설명 없이."""
...
_CATS = ("종목", "산업", "전략", "반도체", "AI", "부동산", "공부", "그외")
...
async def classify_category(title: str, text: str) -> str:
    ...
    word: 종목 / 산업 / 전략 / 반도체 / AI / 부동산 / 공부 / 그외."""
    body = (text or "").strip()[:3000]
    if not body:
        return "그외"
    system = (
        "학습 노트의 종류를 한 단어로만 답하라: "
        "종목 / 산업 / 전략 / 반도체 / AI / 부동산 / 공부 / 그외.\n"
        "- 종목 = 개별 기업/종목에 대한 투자·실적 분석. 반도체 회사를 포함해 "
        "특정 회사를 분석하는 자료면 기술 내용이 많아도 투자/실적 관점이 "
        "핵심이면 종목(반도체·AI보다 우선).\n"
