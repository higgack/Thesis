...
CATEGORY는 노트 내용의 종류를 '주식'/'공부'/'그외' 중 하나로만 적는다.
- 주식 = 주식·투자·트레이딩·증시·종목·기업(실적/투자/정책)·경제·금융 등
  돈·시장과 조금이라도 관련된 모든 것. 투자 전략·매매 기법을 다루면
  책·강의·논문 형태여도 '주식'이다.
- 공부 = 투자·금융과 무관한 순수 학습/지식(기술·과학·공학·역사·자기계발 등).
- 그외 = 둘 다 아닌 것.
애매하면 금융·투자·경제 관련성이 조금이라도 있으면 '주식'으로 한다.
단어 하나만, 설명 없이."""
...
async def classify_category(title: str, text: str) -> str:
    ...
    if not body:
        return "그외"
    system = (
        "학습 노트의 종류를 한 단어로만 답하라: 주식 / 공부 / 그외.\n"
        "- 주식 = 주식·투자·트레이딩·증시·종목·기업(실적/투자/정책)·경제·금융 등 "
        "돈·시장과 조금이라도 관련된 모든 것. 투자 전략·매매 기법이면 책·논문·"
        "강의 형태여도 주식.\n"
        "- 공부 = 투자·금융과 무관한 순수 학습/지식(기술·과학·공학·역사 등).\n"
        "- 그외 = 둘 다 아닌 것.\n"
        "애매하면 금융·투자·경제 관련성이 조금이라도 있으면 주식. "
        "다른 말 없이 단어 하나만.")
    user = f"[제목] {title}\n[본문 일부]\n{body}"
    try:
