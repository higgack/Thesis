...
CATEGORY는 노트 내용의 종류를 '주식'/'반도체'/'AI'/'대학원'/'공부'/'그외'
중 하나로만 적는다. 우선순위(위에서부터 먼저 적용):
- 주식 = 주식·종목·개별 기업/회사 분석·기업 실적·투자/매매 전략·증시.
  반도체 회사를 포함해 특정 회사를 분석하는 자료면 기술 내용이 많아도
  '주식'이다(투자/실적 관점이 핵심이면 반도체보다 주식 우선).
...
- 공부 = 위 어디에도 안 속하는, 특정 기업·투자와 무관한 순수 지식·기술·
  과학·학문 학습.
- 그외 = 부동산, 일반 거시경제·통화·정책·통계 등 위 다섯 어디에도 안
  맞는 것.
단어 하나만, 설명 없이."""
...
async def classify_category(title: str, text: str) -> str:
    ...
    system = (
        ...
        "기술·과학·학문 학습.\n"
        "- 그외 = 부동산, 일반 거시경제·통화·정책·통계 등 위 다섯 어디에도 "
        "안 맞는 것.\n"
        "다른 말 없이 단어 하나만.")
    user = f"[제목] {title}\n[본문 일부]\n{body}"
    try:
