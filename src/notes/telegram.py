...
• <b>중복은 합성 전에 걸러짐</b>(비용 0): 같은 URL·파일명이거나 본문이 같으면 새 노트를 안 만들고 <b>♻️ 이미 노트가 있어 건너뜀</b>으로 알려줌. 추적 파라미터만 다른 URL·이름만 바꾼 파일도 본문 해시로 잡고, <b>붙여넣은 글도 길이와 상관없이</b> 같은 글이면 잡음(2026-09-19 수정 — 그 전에는 짧은 글이 그냥 통과했음). 두뇌(RAG)에 이미 있는 자료는 <b>막지 않음</b> — 보관용과 되새김질용은 별개라 일부러 그렇게 뒀음.
...
목록은 <b>월 → 일로 접혀</b> 나오고 최신 날짜만 펼쳐져 있어(⊕ 전체 펼치기로 한 번에). 검색·필터를 걸면 해당 섹션이 자동으로 펼쳐지고 0건인 날짜는 숨는다. 종류별·유형별 정렬을 고르면 날짜 묶음 없이 평탄하게 보인다.

<b>5. 비용</b>
노트당 flash 합성 ~₩수(저렴) · 파싱 무료(로컬) · OCR 페이지만 유료.
드물게 모델이 같은 문자를 반복하며 폭주하면 1회만 다시 합성해(비용 2배,
노트별 💰에 합산). 재시도도 폭주하면 더 나은 쪽을 저장하고 멈춘다."""
