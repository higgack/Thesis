...
def delete_docs(doc_ids: list[str]) -> int:
    ...
    # Batched IN(...) instead of one DELETE per doc — chunk_fts.doc_id is
    # UNINDEXED (FTS5 gives no secondary index there), so N per-doc
    # deletes meant N full scans of the whole keyword table.
    _meta.fts_delete_docs(list(doc_ids))
    return len(ids)
