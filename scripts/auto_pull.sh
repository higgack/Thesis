...
# Auto-restart hang watchdog (phase 2, 2026-08). Crashes are recovered by
# compose 'restart: unless-stopped'; this covers the hung-but-alive case
# (process up, event loop wedged, heartbeat frozen). Phase 1 (alert-only)
# proved false-positive-free since 2026-06, so phase 2 re-adds the
# auto-restart. All the original false-positive guards (deploy-window mute,
# container-uptime guard, cooldown) are kept unchanged.
...
check_heartbeat() {
    ...
    # restart-loop every healthy deploy. Only judge a container hung once
    # it has been up at least as long as the stale window.
    ...
    # Missing file: bot has not stamped yet (fresh boot / pre-watchdog
    # image). Do not act, so a healthy-but-young container is never
    # restart-looped.
    ...
    # Cooldown: restart + re-alert at most every 30 min while the hang
    # persists to avoid restart thrashing.
    ...
    notify "WARNING: heartbeat ${mins}min stale -- auto-restarting bot"
